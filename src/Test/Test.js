...
const Test = () => {
    ...
    useEffect(() => {
        const get = async () => {
            ...
            const ansArray = response.questions ? response.questions.map((oneObj, index) => {
                ...
                let choosedAns = Array(oneObj.correctOptionIndex.length).fill('');
    ...
    const onSubmit = () => {
        let res = window.confirm("Do you really want to leave?");
        if (res !== true)
            return;
    ...
    const getOptions = (type, questions, qindex) => {
        let optionsList = questions.options.map((oneOption, index) => {
            return (
                <div className={type} key={index}>
                    <label key={index}>
                        <input
                            id={index}
                            type={type}
                            name={type === 'radio' ? 'option' : `op${index}`}
                            value={oneOption}
                            checked={questions.initialValue[index]}
                            onChange={() => onSelect(questions, qindex, index, type)}
                        />{oneOption}
                    </label>
                </div>
            )
    ...
    const renderData = (question, sindex) => {
        if (question) {
            let type = question[0].type ? 'checkbox' : 'radio';
            let optionsList = getOptions(type, question[0], sindex)
                    ...
                    <div className="col-md-12">
                        <div className="panel panel-default">
                            ...
                            <div className="panel-body">
                                ...
                                {renderData(currentItem, indexOfFirstItem, indexOfLastItem)}
